#include "SDL_gesture.h"
 ...
 bool SDLCALL GestureEventWatch(void *userdata, SDL_Event *event)
{
...
void Gesture_Quit(void)
{
 ...
 unsigned long GestureHashDollar(SDL_FPoint *points)
{
...
Returns the index of the gesture on success, or -1 */
 int GestureAddDollar_one(GestureTouch *inTouch, SDL_FPoint *path)
{
...
/* `path` contains raw points, plus (possibly) the calculated length */
 int GestureDollarNormalize(const GestureDollarPath *path, SDL_FPoint *points, bool is_recording)
{
    int i;
    float interval;
    float dist;
 ...
 float GestureDollarRecognize(const GestureDollarPath *path, int *bestTempl, GestureTouch *touch)
{
